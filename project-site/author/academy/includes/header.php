...
if (!empty($_SESSION['logged_id'])) {
    if ($_SESSION['user_cat'] === 2) {
        ...
        $sql = "SELECT ua.university_name uv_id,
               ul.university_name,
               ua.department_category department
        FROM users_academy ua
                 INNER JOIN university_list ul on ua.university_name = ul.id
        WHERE ua.id = '" . get_escape_sql($_SESSION['logged_id']) . "'";
        $result = get_query($sql);
        if (get_row_count($result)) {
            $row = get_fetch_array($result);
            $academy_name = $row['university_name'];
        ...
        <ul class="navbar-nav ml-auto">
            <li class="nav-item">
                <a class="nav-link" href="#0">
                    ...
                    <span class="nav-link-text"><?php echo $academy_name; ?></span>
                </a>
            </li>
            <li class="nav-item">
